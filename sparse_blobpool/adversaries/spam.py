...
class SpamAttackConfig(AttackConfig):
    """Configuration for spam attack.

    Attributes:
        spam_rate: Transactions per second to inject.
        valid_headers: If True, spam txs have valid structure (cause fetches).
        provide_data: If False, spam txs have unavailable data (DoS nodes).
        target_nodes: Specific nodes to target, or None to broadcast to all.
    """
...
class SpamAdversary(Adversary):
    ...
    def _schedule_next_spam(self) -> None:
        """Schedule the next spam transaction."""
    ...
    def _inject_spam(self) -> None:
        """Create and announce a spam transaction."""
