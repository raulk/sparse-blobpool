...
class TargetedPoisoningConfig(AttackConfig):
    """Configuration for targeted poisoning attack.

    Attributes:
        victim_id: The node to target with the attack.
        num_attacker_connections: Number of attacker nodes to connect to victim (k).
        nonce_chain_length: Length of the nonce chain to create.
        injection_interval: Time between injecting chain transactions.
        sender_address: Address to use for all poisoning transactions.
    """
...
class TargetedPoisoningAdversary(Adversary):
    ...
    def execute(self) -> None:
        """Start the targeted poisoning attack."""
    ...
    def _inject_next_tx(self) -> None:
        """Inject next transaction in the nonce chain."""
    ...
    def _create_poison_tx(self) -> TxHash:
        """Create a unique poison transaction hash."""
    ...
    def get_attack_progress(self) -> dict[str, int | float]:
        """Return current attack progress metrics."""
        return {
